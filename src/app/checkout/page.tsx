...
import { ShoppingCart, CreditCard, CheckCircle, ArrowRight, ArrowLeft, Package, Lock, Camera, Gift, AlertCircle } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'scanner' | 'subscription'>('scanner');
  ...
  // Timer effect for scanner countdown
  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (scannerActive && timeRemaining > 0) {
      interval = setInterval(() => {
        setTimeRemaining((prev) => prev - 1);
      }, 1000);
    } else if (timeRemaining === 0 && scannerActive) {
      setScannerActive(false);
      setErrors({ scanner: 'Scan time expired. Please try again.' });
    }
    return () => clearInterval(interval);
  }, [scannerActive, timeRemaining]);
  ...
  const validatePayment = () => {
    ...
    if (paymentMethod === 'scanner' && !scannerActive) {
      newErrors.scanner = 'Please start scanning to proceed';
    }
          ...
          {currentStep !== 4 ? (
                ...
                <div className="lg:col-span-2">
                  ...
                  {currentStep === 2 && (
                    <div className="card p-8">
                      ...
                      {/* Scanner Payment Option */}
                      <div className="mb-8">
                        <label className="flex items-center p-4 border-2 rounded-lg cursor-pointer transition-colors"
                          style={{
                            borderColor: paymentMethod === 'scanner' ? '#7a9e4f' : '#e0d4c0',
                            backgroundColor: paymentMethod === 'scanner' ? '#f5f9f2' : 'white'
                          }}
                        >
                          <input
                            type="radio"
                          ...
                          <div className="ml-4 flex-1">
                            <div className="flex items-center gap-2">
                              <Camera className="w-5 h-5 text-botanical-600" />
                              <h3 className="font-semibold text-soil-800">Scan & Pay</h3>
                            </div>
                            <p className="text-sm text-soil-600 mt-1">Scan QR code with your payment app</p>
                          </div>
                        </label>
                        ...
                        {paymentMethod === 'scanner' && (
                          <div className="mt-6 space-y-4">
                            {/* Scanner Area */}
                            <div className="bg-soil-900 rounded-lg p-8 flex flex-col items-center justify-center h-80">
                              {!scannerActive ? (
                                <>
                                  <Camera className="w-16 h-16 text-white mb-4 opacity-50" />
                                  <p className="text-white text-center mb-4">Click below to start scanning</p>
                                  <Button
                                    variant="primary"
                                    size="lg"
                                    onClick={() => {
                                ...
                                <>
                                  <div className="text-center">
                                    <div className="mb-4">
                                      <p className="text-white text-lg font-semibold">Scanner Active</p>
                                      <p className="text-botanical-300 text-2xl font-bold mt-2">{formatTime(timeRemaining)}</p>
                                    </div>
                                    <div className="w-40 h-40 bg-white bg-opacity-10 rounded-lg border-2 border-botanical-300 flex items-center justify-center mb-4">
                                      <p className="text-white text-sm text-center">Position QR Code<br/>within frame</p>
                                    </div>
                                    <Button
                                      variant="outline"
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
                              ...
                              <p className="text-sm text-blue-700">
                                Payment confirmation will be updated on your order page within 1 hour.
                              </p>
                            </div>
                            ...
                            {errors.scanner && (
                              <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex gap-3">
                                ...
                                <p className="text-sm text-red-700">{errors.scanner}</p>
                              </div>
                            )}
                          </div>
                        )}
                      ...
                      <div>
                        <label className="flex items-center p-4 border-2 rounded-lg cursor-pointer transition-colors"
                          style={{
                            borderColor: paymentMethod === 'subscription' ? '#7a9e4f' : '#e0d4c0',
                            backgroundColor: paymentMethod === 'subscription' ? '#f5f9f2' : 'white'
                          }}
                        >
                          <input
                            type="radio"
                          ...
                          <div className="ml-4 flex-1">
                            <div className="flex items-center gap-2">
                              <Gift className="w-5 h-5 text-botanical-600" />
                              <h3 className="font-semibold text-soil-800">Subscription Plan</h3>
                            </div>
                            <p className="text-sm text-soil-600 mt-1">Monthly recurring subscription</p>
                          </div>
                          <div className="text-right">
                            <p className="font-semibold text-soil-800">₹999/month</p>
                            <p className="text-xs text-soil-500">Placeholder price</p>
                          </div>
                        </label>
                        ...
                        {paymentMethod === 'subscription' && (
                          <div className="mt-6 space-y-4">
                            <div className="bg-botanical-50 border border-botanical-200 rounded-lg p-4">
                              <h4 className="font-semibold text-soil-800 mb-3">Subscription Benefits</h4>
                              <ul className="space-y-2 text-sm text-soil-700">
                                <li className="flex items-center gap-2">
                                  <span className="text-botanical-600">✓</span>
                                  <span>Monthly soil testing kit</span>
                                </li>
                                <li className="flex items-center gap-2">
                                  <span className="text-botanical-600">✓</span>
                                ...
                                <li className="flex items-center gap-2">
                                  <span className="text-botanical-600">✓</span>
                                  <span>Exclusive farming tips & updates</span>
                                </li>
                              </ul>
                            </div>
                          ...
                          <div className="bg-sand-50 rounded-lg p-4 text-sm text-soil-700">
                            {paymentMethod === 'scanner' ? (
                              <>
                                ...
                                <p className="text-xs text-soil-600 mt-2">QR code scanner payment method</p>
                              </>
                            ) : (
                              <>
                                <div className="flex items-center gap-2">
                                ...
                                <p className="text-xs text-soil-600 mt-2">₹999/month (placeholder price)</p>
                              </>
                            )}
                          </div>
                        </div>
            ...
            <div className="max-w-2xl mx-auto text-center">
              <div className="card p-12">
                ...
                <p className="text-lg text-soil-600 mb-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-8 flex gap-3">
                  <AlertCircle className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                  <p className="text-sm text-blue-700">
                    Your payment status will be confirmed on your order page within 1 hour.
                  </p>
                </div>
